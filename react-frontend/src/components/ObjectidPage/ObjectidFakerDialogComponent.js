...
import objectidFakerFactory from "./objectidFakerFactory";
...
const ObjectidFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = objectidFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("objectid").create(elem));
...
export default connect(null, mapDispatch)(ObjectidFakerDialogComponent);
